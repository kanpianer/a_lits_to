        ...
        if (scrollState.speed !== 0 && !scrollState.raf) {
          const scrollLoop = () => {
            if (scrollState.speed !== 0 && isDragging.current) {
              container.scrollTop += scrollState.speed;
...
content = content.replace(/  const startDrag = \(e: React\.PointerEvent\) => \{[\s\S]*?  \};/, startDragReplacement.trim());
